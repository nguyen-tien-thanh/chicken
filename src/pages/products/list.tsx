...
import { PRODUCT_TYPE_LABELS, type IProduct } from '@/types';
...
export const List = () => {
  ...
  const { filters, sorters, setFilters } = useTable<IProduct>({
    resource: 'products',
      ...
      {isError ? (
        ...
        >
          <MobileShowList
            dataSource={records}
            getKey={row => row.id}
            onItemClick={row => show('products', row.id)}
            renderTitle={row => row.name}
            renderDescription={row =>
              [PRODUCT_TYPE_LABELS[row.type], row.category?.name]
                .filter(Boolean)
                .join(' · ')
            }
          />
